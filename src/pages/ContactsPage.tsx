...
import { RAW_STATUSES, STATUS_LABELS } from '@/lib/status';
...
function buildQuery(q: string, status: string, page: number, limit: number, role?: UserLite['role']) {
    ...
    if (role === 'sales') p.set('assignedToSelf', '1'); // server will filter to own (and optionally unassigned)
    return p;
}
...
function ConfirmDialog(props: {
    ...
    const { open, title, message, confirmText = 'Ооба, тастыктайм', cancelText = 'Жокко чыгаруу', loading = false, onConfirm, onCancel } = props;
    if (!open) return null;
    return (
            ...
            <div className="absolute inset-0 flex items-center justify-center p-4">
                <div className="w-full max-w-md rounded-2xl shadow-xl z-[9999] border bg-white border-gray-200 dark:bg-gray-900 dark:border-gray-800">
                    ...
                    <div className="p-4 border-t border-gray-200 dark:border-gray-800 flex justify-end gap-2">
                        <Button variant="ghost" onClick={onCancel} disabled={loading} size="sm">{cancelText}</Button>
                        <Button variant="danger" onClick={onConfirm} disabled={loading} size="sm">{loading ? 'Аткарылууда…' : confirmText}</Button>
                    </div>
                </div>
            </div>
        </div>
...
export default function ContactsPage() {
    ...
    const load = useCallback(async () => {
        ...
        try {
            ...
            setData(data); // rely on server filtering for sales
    ...
    function askDelete(c: Contact) { setTarget(c); setConfirmOpen(true); }
    ...
    async function confirmDelete() {
        ...
        try {
            ...
            setConfirmOpen(false); setTarget(null); setConfirmLoading(false);
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="md:hidden sticky top-14 z-30 -mx-4 px-4 py-2 bg-white/85 dark:bg-gray-900/85 backdrop-blur
        supports-[backdrop-filter]:bg-white/60 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between">
                <h1 className="text-base font-semibold truncate">{t.contacts.title}</h1>
                <Button variant="primary" onClick={() => setNewOpen(true)} aria-label="Жаңы лид" title="Жаңы лид" size="sm">
                    {createLabel}
                </Button>
            </div>
            ...
            <div className="hidden md:flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                    {selected.length > 0 && isSuperadmin && (
                        <>
                            <Button variant="danger" onClick={openBulkDelete}>Тандалгандарды өчүрүү ({selected.length})</Button>
                            <Button variant="danger" onClick={openBulkPurge}>Тандалгандарды түбөлүк өчүрүү</Button>
                        </>
                    )}
                    <Button variant="primary" onClick={() => setNewOpen(true)}>{createLabel}</Button>
                </div>
            </div>
                ...
                <CardBody>
                    <div className="md:hidden space-y-2">
                        <div className="relative">
                            <Input value={typedQ} onChange={(e) => { setPage(1); setTypedQ(e.target.value); }}
                                placeholder="Аты, email же телефон" aria-label="Издөө" />
                            {typedQ && (
                                <button type="button" className="absolute right-8 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-700 text-sm"
                                    aria-label="Тазалоо" onClick={() => { setTypedQ(''); setPage(1); }}>×</button>
                            )}
                        ...
                        <div id="mobile-filters" className='grid grid-cols-2 gap-2'>
                            <Select value={status} onChange={(e) => { setPage(1); setStatus(e.target.value); }} aria-label="Статус боюнча чыпкалоо">
                                {RAW_STATUSES.map((s) => (<option key={s || 'ALL'} value={s}>{STATUS_LABELS[s]}</option>))}
                            </Select>
                ...
                <CardBody>
                        ...
                        <Table>
                                ...
                                <tr className="border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800">
                                    {isSuperadmin && (
                                        <th className="px-3 py-2 text-left text-sm font-semibold bg-gray-50 text-gray-700 dark:bg-gray-800 dark:text-gray-200">
                                            <input type="checkbox" checked={allOnPageSelected} onChange={(e) => toggleSelectAll(e.target.checked)} aria-label="Баарын тандоо" />
                                        </th>
                                    )}
                            ...
                            <TBody>
                                    ...
                                    <tr>
                                        <td colSpan={colCount} className="p-8">
                                            <div className="text-center space-y-3">
                                                <div className="text-gray-500 dark:text-gray-400">{t.contacts.empty}</div>
                                                <Button variant="primary" onClick={() => setNewOpen(true)}>Жаңы лид кошуу</Button>
                                            </div>
                                        </td>
                                    </tr>
                                )}
                                {data?.items?.map((c) => {
                                    ...
                                    return (
                                        <tr key={c.id} className="border-t hover:bg-gray-50 dark:hover:bg-gray-800/60 cursor-pointer transition-colors"
                                            ...
                                            {isSuperadmin && (
                                                <td onClick={(e) => e.stopPropagation()}>
                                                    <input type="checkbox" checked={selected.includes(c.id)}
                                                        onChange={(e) => { if (e.target.checked) setSelected((s) => [...s, c.id]); else setSelected((s) => s.filter((id) => id !== c.id)); }}
                                            ...
                                            <td>
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    {typeof c.contactAttempts === 'number' && c.contactAttempts > 0 && (
                                                        <span title={`Байланыш аракеттери: ${c.contactAttempts}`}
                                                            className="inline-flex items-center px-2 py-0.5 text-xs rounded-full border bg-gray-50 border-gray-200 text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-100">
                                                            {c.contactAttempts}
                                                        </span>
                                                    )}
                                                </div>
                                            ...
                                            <td className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
                                                {(isAssistant || isManager || isAdmin || isSuperadmin) && (
                                                    <div className="flex items-center gap-2">
                                                        <Select value={c.assignedToUserId ? String(c.assignedToUserId) : ''}
                                                            onChange={(e) => onAssignChange(c.id, e.target.value)}
                                                            disabled={busy || assignables.length === 0}
                                                            title="Жооптуу адам" aria-label="Жооптуу адам">
                                                            <option value="">{busy ? 'Жүктөлүүдө...' : '— (Дайындалган жок)'}</option>
                                                            {assignables.map((u) => (<option key={u.id} value={u.id}>{u.fullName}</option>))}
                                                        </Select>
                                                        {busy && <span className="text-xs opacity-70">…</span>}
                                                    </div>
                                                )}
                                                {isSales && isUnassigned && (
                                                    <Button variant="primary" disabled={busy} onClick={() => onSelfAssign(c.id)} size="sm">
                                                        {busy ? 'Жүктөлүүдө…' : 'Өзүмө алуу'}
                                                    </Button>
                                                )}
                                                {(isAdmin || isSuperadmin) && (
                                                    <Button variant="danger" onClick={() => askDelete(c)} aria-label="Өчүрүү" title="Өчүрүү" size="sm">
                                                        <Trash2 size={16} />
                                                    </Button>
                                                )}
                                            </td>
                    ...
                    <div className="hidden md:flex items-center justify-between mt-3">
                        ...
                        <div className="flex gap-2">
                            <Button variant="ghost" disabled={!canPrev} onClick={() => setPage((p) => p - 1)} aria-label="Мурунку бет" title="←" size="sm">
                                <ChevronLeft size={16} />
                            </Button>
                            <Button variant="ghost" disabled={!canNext} onClick={() => setPage((p) => p + 1)} aria-label="Кийинки бет" title="→" size="sm">
            ...
            <div className="lg:hidden grid grid-cols-1 md:grid-cols-2 gap-2">
                ...
                {!loading && (data?.items?.length ?? 0) === 0 && (
                    <Card><CardBody><div className="p-6 text-center text-gray-500 dark:text-gray-400">{t.contacts.empty}</div></CardBody></Card>
                )}
                ...
                {data?.items?.map((c) => {
                    ...
                    const prov = c.sourceProvider?.trim();
                    return (
                        <Card key={c.id} className="overflow-hidden cursor-pointer" onClick={() => nav(`/contacts/${c.id}`)}>
                            <div className="flex items-center justify-between px-3 py-2 border-b border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/60">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    {isSuperadmin && (
                                        <>
                                            <label className="hidden md:flex items-center gap-1 text-[11px]" onClick={(e) => e.stopPropagation()}>
                                            ...
                                            <Button variant="danger" aria-label="Өчүрүү" title="Өчүрүү"
                                                onClick={(e) => { e.stopPropagation(); askDelete(c); }} size="sm">
                                                <Trash2 size={16} />
                                            </Button>
                                        </>
                                    )}
                            ...
                            <CardBody className="p-3">
                                ...
                                <div className="mt-2 flex flex-wrap items-center gap-2" onClick={(e) => e.stopPropagation()}>
                                    {(isAssistant || isManager || isAdmin || isSuperadmin) && (
                                        <Select value={c.assignedToUserId ? String(c.assignedToUserId) : ''}
                                            onChange={(e) => onAssignChange(c.id, e.target.value)}
                                            disabled={busy} aria-label="Жооптуу адам">
                                            <option value="">{busy ? 'Жүктөлүүдө...' : '— Дайындалган жок'}</option>
                                            {assignables.map((u) => (<option key={u.id} value={u.id}>{u.fullName}</option>))}
                                        </Select>
                                    )}
                                    {isSales && isUnassigned && (
                                        <Button variant="primary" disabled={busy}
                                            onClick={(e) => { e.stopPropagation(); onSelfAssign(c.id); }} size="sm">
                                            {busy ? 'Жүктөлүүдө…' : 'Өзүмө алуу'}
                                        </Button>
                                    )}
                                </div>
            ...
            <div className="lg:hidden flex items-center justify-between py-2">
                <Button variant="ghost" disabled={!canPrev} onClick={() => setPage((p) => p - 1)} aria-label="Мурунку бет" size="sm">
                    <ChevronLeft size={16} />
                </Button>
                <div className="text-sm">{t.contacts.page}: {data?.page ?? 1} / {totalPages}</div>
                <Button variant="ghost" disabled={!canNext} onClick={() => setPage((p) => p + 1)} aria-label="Кийинки бет" size="sm">
                    <ChevronRight size={16} />
                </Button>
            </div>
            ...
            {isSuperadmin && hasSelection && (
                <div className="md:hidden fixed left-0 right-0 bottom-0 z-40 border-t bg-white/95 dark:bg-gray-900/95 backdrop-blur px-3 py-3 flex items-center justify-between"
                    style={{ paddingBottom: 'max(env(safe-area-inset-bottom), 0px)' }}>
                    <div className="text-sm">{selected.length} тандалды</div>
                    <div className="flex gap-2">
                        <Button variant="danger" onClick={openBulkDelete} size="sm">Өчүрүү</Button>
                        <Button variant="danger" onClick={openBulkPurge} size="sm">Түбөлүк өчүрүү</Button>
                    </div>
                </div>
            )}
    ...
    async function onAssignChange(contactId: number, assigneeStr: string) {
        ...
        setData((prev) => {
            if (!prev) return prev;
            return { ...prev, items: prev.items.map(it => it.id === contactId ? { ...it, assignedToUserId: assigneeUserId } : it) };
        });
        try {
    ...
    async function onSelfAssign(contactId: number) {
        ...
        setData((prev) => {
            ...
            return { ...prev, items: prev.items.map(it => it.id === contactId ? { ...it, assignedToUserId: myId } : it) };
        });
        try {
