...
const schema = z.object({
  ...
  // NEW: assignee comes from a <select> as a string ('' | userId)
...
type Assignable = { id: number; fullName: string };
...
type Props = {
  ...
  // NEW: pass assignable users from parent (assistant/manager/admin/superadmin typically see this)
  assignables?: Assignable[];
};

export default function NewLeadModal({ open, onClose, onCreated, assignables = [] }: Props) {
  ...
  const onSubmit: SubmitHandler<FormValues> = async (values) => {
    // If sales → force self-assign; else respect selected value (or null)
    const assignedId =
      isSales
        ? Number(user?.id)
        : (values.assignedToUserId && values.assignedToUserId !== '' ? Number(values.assignedToUserId) : null);
    ...
    const payload = {
      ...
      assignedToUserId: assignedId, // ← enforce self-assign for sales
    } as const;

    try {
      const { data } = await api.post('/contacts/manual', payload);
      const who = data?.assignedToName
        || (assignedId ? (isSales ? user?.fullName : assignables.find(a => a.id === assignedId)?.fullName) : null);

      toast.push({
        title: 'OK',
        message: who
          ? `Лид ийгиликтүү кошулду. Жооптуу: ${who}.`
    ...
    >
      <form id="new-lead-form" onSubmit={handleSubmit(onSubmit)} className="space-y-4 md:space-y-6">
        ...
        {/* source provider (conditional) */}
          ...
          {/* NEW: Assignee selector (shown when assignables provided) */}
          {!isSales && assignables.length > 0 && (
            <Field label="Жооптуу адам">
              <Select {...register('assignedToUserId')}>
                <option value="">{'— (Дайындалган жок)'}</option>
                {assignables.map((u) => (
